...
- `Get` retrieves and reconstructs the original Fibre blob from FSPs by blob ID.
  It does not require a height parameter because the validator set is not
  expected to change enough during the blob's retention period to affect the
  ability to download blobs uploaded before the change.
